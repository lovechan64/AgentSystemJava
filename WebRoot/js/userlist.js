$().ready(function(){
		...
		if(roleid==1){
		 ...
		 if(confirm("您确定要删除【"+userName+"】吗？")) {
			 ...
			 var add='delete';
				...
				$.post("/edituser.action",{'user.id':id,'type':add}
				,function(result){
	...
	$("#addUserSubmit").click(function(){
		...
		if(a_userCode==''){
			...
			var add='add';
			$.post("/edituser.action",{'type':add,'user.userName':a_userName,'user.userPassword':a_userPassword,'user.roleId':a_roleId,'user.isStart':a_isStart,'user.userCode':a_userCode}
			,function(result){
	...
	$("#modifyUserSubmit").click(function(){
		...
		if(userCode==''){
			...
			$.post("/edituser.action",{'user.id':id,'type':add,'user.userName':userName,'user.userPassword':userPassword,'user.roleId':roleId,'user.isStart':isStart,'user.userCode':userCode}
			,function(result){
